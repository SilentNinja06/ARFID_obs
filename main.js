...
// ../../packages/core/dist/symptoms.js
var SYMPTOM_HEADING = "Symptoms";
var SYMPTOM_MARKER = "%% symptom-log %%";
function formatSymptomLine(e) {
  const time = e.time ? `${e.time} ` : "";
  const note = e.note && e.note.trim() ? ` \u2014 ${e.note.trim()}` : "";
  const src = e.source ? ` _(${e.source})_` : "";
  return `- ${time}${e.symptoms.join(", ")}${note}${src}`;
}
...
var ArfidSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  addListSetting(name, desc, rows, get, set) {
    new import_obsidian3.Setting(this.containerEl).setName(name).setDesc(desc).setClass("arfid-list-setting").addTextArea((t) => {
      t.setValue(get().join("\n")).onChange(async (v) => {
        set(
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Daily note linking").setHeading();
    new import_obsidian3.Setting(containerEl).setName("Link entries into the daily note").setDesc("Insert a link into that day's daily note whenever an entry is logged. New daily notes are seeded from the Daily Notes core plugin's template.").addToggle(
    ...
    new import_obsidian3.Setting(containerEl).setName("Quick-log chip lists").setHeading();
    this.addListSetting(
      "Known strategies",
    ...
    new import_obsidian3.Setting(containerEl).setName("Support & reminders").setHeading();
    this.addListSetting(
      "Kindness reminders",
...
async function linkIntoDailyNote(app, settings, date, time, noteBasename, label) {
  ...
  const dailyName = (0, import_obsidian4.moment)(date, "YYYY-MM-DD").format(format);
  const path = (0, import_obsidian4.normalizePath)((folder ? folder + "/" : "") + dailyName + ".md");
...
function insertUnderHeading(content, line, marker, heading, time) {
  const lines = content.split("\n");
  if (lines.some((l) => l.trim() === line.trim())) return content;
  let anchor = marker ? lines.findIndex((l) => l.includes(marker)) : -1;
  if (anchor === -1 && heading) {
...
`;
  ...
  let at = anchor + 1;
  const LOG = /^- \d{2}:\d{2} /;
  while (at < lines.length && LOG.test(lines[at])) {
    if (lines[at].slice(2, 7) > time) break;
    at++;
  }
  lines.splice(at, 0, line);
  return lines.join("\n");
}
async function appendSharedSymptomLine(app, date, time, line) {
  var _a;
  const opts = getDailyNotesOptions(app);
  const format = opts.format || "YYYY-MM-DD";
  const folder = ((_a = opts.folder) != null ? _a : "").trim().replace(/\/+$/, "");
  ...
  if (!(file instanceof import_obsidian4.TFile)) return;
  await app.vault.process(file, (content) => insertUnderHeading(content, line, SYMPTOM_MARKER, SYMPTOM_HEADING, time));
}
...
async function ensureFolder(app, folder) {
  ...
  if (!f) return;
  if (app.vault.getAbstractFileByPath((0, import_obsidian5.normalizePath)(f)) instanceof import_obsidian5.TFolder) return;
  await app.vault.createFolder((0, import_obsidian5.normalizePath)(f)).catch(() => {
  });
}
async function createUniqueNote(app, folder, base, content) {
  const f = normalizeFolder(folder);
  await ensureFolder(app, f);
  let path = (0, import_obsidian5.normalizePath)((f ? f + "/" : "") + base + ".md");
  let n = 1;
  while (app.vault.getAbstractFileByPath(path)) {
    path = (0, import_obsidian5.normalizePath)((f ? f + "/" : "") + `${base} ${++n}.md`);
  }
...
async function saveEntryNote(plugin, date, time, food, content, opts = {}) {
  ...
  if (opts.notice) new import_obsidian5.Notice(opts.notice);
...
var ArfidModal = class extends import_obsidian6.Modal {
...
var StatusChangeModal = class extends ArfidModal {
  ...
  async save() {
    ...
    if (!this.status) {
      new import_obsidian7.Notice("Pick the new category.");
      return;
    }
    const known = findFood(this.foods, food);
    if (known && known.currentStatus === this.status) {
      new import_obsidian7.Notice(`${food} is already marked ${STATUS_LABELS[this.status].toLowerCase()}.`);
      return;
    }
...
var SymptomModal = class extends ArfidModal {
  ...
  async save() {
    if (this.symptoms.size === 0) {
      new import_obsidian10.Notice("Pick at least one symptom.");
      return;
    }
    const { date, time } = nowStamp();
    ...
    try {
      await appendSharedSymptomLine(this.app, date, time, formatSymptomLine({ symptoms, note: this.notes, source: "nourishment", time }));
    } catch (e) {
      console.error("ARFID Tracker: shared symptom log failed", e);
    }
    this.close();
  }
};
...
var FoodNoteModal = class extends ArfidModal {
  ...
  async save() {
    const food = this.foodName.trim();
    if (!food) {
      new import_obsidian11.Notice("Add a food name first.");
      return;
    }
    if (!this.body.trim()) {
      new import_obsidian11.Notice("Write the details first \u2014 that's the part future you needs.");
      return;
    }
    const { date } = nowStamp();
    ...
    new import_obsidian11.Notice(`Saved ${NOTE_KIND_LABELS[this.kind].toLowerCase()} for ${food}.`);
    this.close();
...
var AddFoodModal = class extends ArfidModal {
  ...
  async save() {
    const food = this.foodName.trim();
    if (!food) {
      new import_obsidian12.Notice("Add a food name first.");
      return;
    }
    ...
    if (!this.status) {
      new import_obsidian12.Notice("Pick a category for it.");
      return;
    }
    const { date, time } = nowStamp();
...
var AddFoodsModal = class extends ArfidModal {
  ...
  async save() {
    ...
    if (added === 0 && skipped.length === 0) {
      new import_obsidian12.Notice("Nothing to add \u2014 enter some foods first.");
      return;
    }
    ...
    new import_obsidian12.Notice(msg, skipped.length > 0 ? 8e3 : 4e3);
    this.close();
  }
};
...
async function writeExport(app, settings, filename, content) {
  ...
  const path = (0, import_obsidian13.normalizePath)((folder ? folder + "/" : "") + filename);
...
async function exportSummary(app, settings, store) {
  ...
  new import_obsidian13.Notice(`Summary written to ${file.path}`);
...
async function deleteEntry(plugin, entry) {
  ...
  new import_obsidian14.Notice(`Deleted \u201C${entry.food}\u201D.`);
  plugin.notifyDataChanged();
}
async function deleteSymptom(plugin, entry) {
  ...
  await plugin.app.fileManager.trashFile(entry.file);
  new import_obsidian14.Notice("Deleted symptom entry.");
  plugin.notifyDataChanged();
}
async function deleteFood(plugin, food) {
  ...
  for (const note of notes) {
    if (note.file instanceof import_obsidian14.TFile) await plugin.app.fileManager.trashFile(note.file);
  }
  const noteBit = notes.length > 0 ? ` and ${notes.length} note${notes.length === 1 ? "" : "s"}` : "";
  new import_obsidian14.Notice(`Deleted \u201C${food.name}\u201D \u2014 ${food.entries.length} entr${food.entries.length === 1 ? "y" : "ies"}${noteBit}.`);
  plugin.notifyDataChanged();
}
...
var ArfidDashboardView = class extends import_obsidian15.ItemView {
...
// src/main.ts
var ArfidTrackerPlugin = class extends import_obsidian16.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      logDrift(collectWarnings(notes, (t) => t === "food-entry"), "arfid-tracker");
    });
